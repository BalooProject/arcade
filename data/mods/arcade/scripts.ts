export const Scripts: ModdedBattleScriptsData = {
		...
		for (const i in this.data.Items) {
			...
			if (item.megaStone) {
				for (const megaEvo of Object.values(item.megaStone)) {
					...
					if (!this.data.FormatsData[megaEvoId]) {
						this.data.FormatsData[megaEvoId] = {};
					}
	...
	actions: {
		...
		getFormeChangeDeltas(formeChangeSpecies, pokemon) {
			let baseSpecies = this.dex.species.get(
				formeChangeSpecies.isMega ?
					formeChangeSpecies.battleOnly as string :
					formeChangeSpecies.baseSpecies
			);
